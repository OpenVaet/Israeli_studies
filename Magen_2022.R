...
df <- data.frame(
  Day = c(0, 7, 14, 21, 28),
  PersonsAtRisk = c(182122, 138003, 113810, 99706, 87330),
  CumulativeEvents = c(0, 3643, 7192, 9902, 11836)
)
print(df)
...
# Create the final_df: fetch Day, CumulativeIncidenceRate from df_intervals
final_df <- df_intervals %>%
  select(Day, CumulativeIncidenceRate)

# Adds a row for Day=28 with the final incidence_rate
final_df <- bind_rows(
  final_df,
  data.frame(Day = 28, CumulativeIncidenceRate = incidence_rate)
)
...
# Writes to CSV
write.csv(final_df, file = "magen_2022.csv", row.names = FALSE)